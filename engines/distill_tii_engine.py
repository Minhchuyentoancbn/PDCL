...
def train_and_evaluate(model: torch.nn.Module, model_without_ddp: torch.nn.Module,
    ...
    for task_id in range(args.num_tasks):
        ...
        if task_id > 0 and (not args.use_gaussian):
            ...
            train_task_adaptive_prediction(model, args, device, class_mask, task_id, data_loader[task_id]['train'], old_head=old_head)
            print('-' * 20)
...
def train_task_adaptive_prediction(model: torch.nn.Module, args, device, class_mask=None, task_id=-1, dataloader=None, old_head=None):
    model.train()
